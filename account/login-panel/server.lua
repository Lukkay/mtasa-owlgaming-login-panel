...
function validateCredentials(username,password,checksave)
	if not (username == "") then
		if not (password == "") then
			...
			triggerClientEvent(client,"set_warning_text",client,"Login","Musíš zadat heslo!")
		end
	else
		triggerClientEvent(client,"set_warning_text",client,"Login","Musíš zadat jméno!")
	end
	return false
end
			...
			if result and client then
				if #result > 0 then
					...
					if exports.bans:checkAccountBan(accountData["id"]) then
						triggerClientEvent(client,"set_warning_text",client,"Účet je zabanován")
					...
					triggerClientEvent(client,"set_authen_text",client,"Login","Heslo přijato, ověřuji..")
					-- Check if old method
					if not string.find(accountData["password"], "$", 1, true) then -- Plain search, not regex
						...
						if accountData["password"] ~= encryptedPW then
							triggerClientEvent(client,"set_warning_text",client,"Login","Heslo je nesprávné pro účet '".. username .."'!")
							return false
						end

						triggerClientEvent(client,"set_authen_text",client,"Login","Kontroluji heslo..")
						...
						if not dbExec(exports.mysql:getConn("core"), "UPDATE `accounts` SET `password`=?, `salt`=NULL WHERE id=?", new_pass, accountData["id"]) then
							triggerClientEvent(client,"set_warning_text",client,"Login","Heslo se nepodařilo změnit pro účet '".. username .."'!")
							return false
						end
					else -- Else if new
						...
						if not verified then
							triggerClientEvent(client,"set_warning_text",client,"Login","Heslo pro účet '".. username .."' je nesprávné.")
							return false
						end
					end

					if tonumber(accountData["activated"]) == 0 then
						triggerClientEvent(client,"set_warning_text",client,"Login","Účet '".. username .."' není aktivován.")
						return false
					end

					--Validation is done, fetching some more details
					triggerClientEvent(client,"set_authen_text",client,"Login","Účet autorizován!")
					...
					for _, thePlayer in ipairs(exports.pool:getPoolElementsByType("player")) do
						...
						if (playerAccountID) then
							if (playerAccountID == tonumber(accountData["id"])) and (thePlayer ~= client) then
								...
								triggerClientEvent(client,"set_authen_text",client,"Login","Účet je aktuálně online.. Odpojuji aktuálního uživatele")
								break
							end
						end
					end
					...
					local result_mta = dbPoll(qb, -1)
					if not result_mta then
						triggerClientEvent(client,"set_authen_text",client,"Login","Magic Data připojení selhalo!")
						return
					elseif #result_mta == 0 then
					...
					setElementDataEx(client, "helper_level", tonumber(accountData['helper']), true)
					...
					if not exports.serialwhitelist:check(client) then
						triggerClientEvent(client,"set_warning_text",client,"Login","Nejsi oprávněn hrát na tomto počítači.")
					...
					triggerClientEvent(client,"set_warning_text",client,"Login","Účet '".. username .."' neexistuje!")
				end
			else
				triggerClientEvent(client,"set_warning_text",client,"Login","Připojení k databázi selhalo!")
				dbFree(qh)
			end
...
function goFromLoginToSelectionScreen(player)
	...
	if (getElementData(player, "appstate") or 0) < 3 then
		if exports.integration:isPlayerGameAdmin(player) or exports.integration:isPlayerTrialGameModerator(player) then
	...
	dbQuery(function(qh, username, password, confirmPassword, email, client)
		...
		if result then
			if #result == 0 then
				...
				if not Q2 then
					triggerClientEvent(client,"set_warning_text",client,"Register","Error code 0003.")
					return false
				end

				local usernameExisted = mysql:fetch_assoc(Q2)
				if (mysql:num_rows(Q2) > 0) and usernameExisted["id"] ~= "1" then
					triggerClientEvent(client,"set_warning_text",client,"Register","Více účtů než jeden není povolen (Existuje: "..tostring(usernameExisted["username"])..")")
					return false
				end
					...
					dbQuery(function(qh, client)
						...
						if result and #result == 1 then
							if dbExec(exports.mysql:getConn("mta"), "INSERT INTO account_details SET `account_id`=?, `mtaserial`=?", result[1].id, mtaSerial) then
								...
								triggerClientEvent(client,"set_warning_text",client,"Register","Error code 0005.")
							end
						else
							triggerClientEvent(client,"set_warning_text",client,"Register","Error code 0004.")
							dbFree(qh)
						end
					end, {client}, exports.mysql:getConn("core"), "SELECT id FROM accounts WHERE username=?", username)
				else
					triggerClientEvent(client,"set_warning_text",client,"Register","Nepodařilo se vytvořit účet.")
					return false
				end
			else
				triggerClientEvent(client,"set_warning_text",client,"Register","Jméno nebo e-mail je již aktivován na jiném účtu.")
			end
		else
			triggerClientEvent(client,"set_warning_text",client,"Register","Error code 0002.")
			dbFree(qh)
		end
